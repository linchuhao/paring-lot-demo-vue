    ...
    <el-table
      ...
      <el-table-column label="operate" min-width="22%">
        <template slot-scope="scope">
          <el-button
            size="mini"
            @click="updateParkingLot(scope.$index, scope.row)">Update
          </el-button>
          <el-button
            size="mini"
            type="danger"
            @click="deleteParkingLot(scope.$index, scope.row)">Delete
          </el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="dialogTitle" :visible.sync="parkingLotInfoVisible" width="45%" @close='cancelDialog()'>
      <el-form :model="parkingLotInfo" label-width="120px" :rules="parkingLotInfoRule" ref="parkingLotInfo">
        ...
        <el-form-item label="Detail Address" prop="address">
          <el-input v-model="parkingLotInfo.address" placeholder="parking lot detail address"
                    autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item label="Capacity" prop="parkingSpaceCapacity">
          <el-input v-model="parkingLotInfo.parkingSpaceCapacity" placeholder="parking lot capacity"
                    autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item label="Remainder" prop="parkingSpaceRemainder">
          <el-input v-model="parkingLotInfo.parkingSpaceRemainder" placeholder="parking lot remainder"
                    autocomplete="off"></el-input>
        </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
            ...
            <el-button v-if="isUpdateParkingLot" type="primary"
                       @click="confirmUpdate('parkingLotInfo')">Confirm</el-button>
          </span>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  data () {
    ...
    const checkParkingSpaceCapacity = (rule, value, callback) => {
      if (!value) {
        return callback(new Error('Parking lot name should be not null'))
      } else {
        callback()
    ...
    const checkParkingSpaceRemainder = (rule, value, callback) => {
      if (!value) {
        return callback(new Error('Parking lot name should be not null'))
      } else {
        callback()
  ...
  destroyed () {
    this.webSock.close() // Disconnect websocket connection after leaving the router
  },
  methods: {
    ...
    initWebSocket () {
      const ws = 'ws://127.0.0.1:8888/websocket'
    ...
    websocketOnOpen () { // After the connection is established, execute the send method to send data
      let actions = {'test': '12345'}
      this.websocketSend(JSON.stringify(actions))
    },
    websocketOnError () { // Connection failed try to reconnect
      this.initWebSocket()
    },
    websocketOnMessage (e) { // Receive data from backend
      const data = JSON.parse(e.data)
      console.log('receive data from backend:')
      console.log(data)
    },
    websocketSend (Data) { // Send data to backend
